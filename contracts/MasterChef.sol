...
import "./PrivacySwap.sol";

// MasterChef is the master of PRV. He can make PRV and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PRV is sufficiently
...
contract MasterChef is Ownable {
    ...
    function pendingPRV(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accPRVPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 prvReward = multiplier.mul(prvPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        prv.mint(devaddr, prvReward.div(10));
        prv.mint(address(this), prvReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safePRVTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
